...
export function deriveGroups(
  providers: {
    id: string;
    name: string;
    kind: 'cloud-api' | 'local-cli' | 'local-service' | 'subscription';
    enabled: boolean;
    config: Record<string, unknown> | null;
  }[],
): ModelGroup[] {
  // Every other kind lists models the same way (config.enabledModels), so only
  // local-cli takes the synthetic single-entry path below.
