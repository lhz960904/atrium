/**
 * Chinese catalog — the source shape every other locale must match (en is typed
 * as `typeof zh`). Keys are grouped by feature. `{{x}}` are interpolation slots.
 */
export const zh = {
  common: {
    delete: '删除',
    edit: '编辑',
    cancel: '取消',
    save: '保存',
    create: '创建',
    close: '关闭',
    download: '下载',
    remove: '移除',
    hide: '隐藏',
    show: '显示',
    loading: '加载中…',
    untitledChat: '未命名对话',
    attachment: '附件',
    listSep: '、',
    copy: '复制',
    fitToView: '适应视图',
    selected: '已选中',
  },
  update: {
    entry: '更新',
    title: '软件更新',
    whatsNew: '更新内容',
    download: '下载',
    downloading: '下载中…',
    downloadComplete: '下载完成，重启即可安装。',
    restartNow: '立即重启',
    restarting: '重启中…',
    later: '稍后',
    failed: '更新失败',
    retry: '重试',
    eta: '剩余 {{eta}}',
  },
  sidebar: {
    running: '运行中',
    unread: '未读',
    scheduled: '已绑定定时任务',
    search: '搜索',
    pinned: '置顶',
    chats: '对话',
    projects: '项目',
    addProject: '添加项目',
    projectNoChats: '暂无对话',
    pin: '置顶',
    unpin: '取消置顶',
    renameProject: '重命名项目',
    archiveProject: '归档项目',
    deleteProject: '删除项目',
    archiveProjectConfirm: '归档此项目及其下全部对话？',
    deleteProjectConfirm: '永久删除此项目及其下全部对话？此操作不可撤销。',
    sortFilter: '排序 / 筛选',
    noChats: '还没有对话',
    settings: '设置',
  },
  modelPicker: {
    placeholder: '选择模型',
    search: '搜索模型…',
    empty: '没有已启用的模型。',
    configure: '去 Settings → Providers 配置 ↗',
    noMatch: '无匹配',
  },
  scheduled: {
    title: '定时任务',
    subtitle: '管理循环任务、提醒与监控',
    searchPlaceholder: '搜索定时任务',
    current: '当前',
    empty: '还没有定时任务。在对话里说，例如「每个工作日早上 8 点，总结 AI 新闻」。',
    selectHint: '选择一个任务查看详情',
    onceAt: '一次性 · {{when}}',
    nextShort: '下次 {{when}}',
    edit: '编辑',
    titleLabel: '任务标题',
    promptLabel: '任务提示词',
    kindRecurring: '循环',
    kindOnce: '一次性',
    cronLabel: 'Cron 表达式',
    pickDateTime: '选择日期和时间',
    atTime: '时间',
    status: '状态',
    statusActive: '进行中',
    statusPaused: '已暂停',
    nextRun: '下次运行',
    lastRan: '上次运行',
    never: '从未',
    details: '详情',
    runsIn: '运行于',
    local: '本地',
    project: '项目',
    noProject: '无',
    repeats: '重复',
    model: '模型',
    defaultModel: '默认',
    previousRuns: '历史运行',
    noRuns: '尚无运行',
    runNow: '立即运行',
    pause: '暂停',
    resume: '恢复',
    delete: '删除',
    openThread: '打开会话',
    deleteConfirm: '删除这个定时任务？它过往的会话会保留。',
    startedToast: '定时任务已开始',
    alreadyRunningToast: '该任务正在运行中',
  },
  composer: {
    placeholder: '说点什么…',
    attach: '附件',
    stop: '停止',
    send: '发送',
    workInProject: '在项目中工作',
    noProject: '不属于项目',
    noProjectDesc: '工具将使用一个临时目录',
    projectsTitle: '项目',
    projectsSubtitle: '所有工具操作都将以此目录作为工作目录',
    selectDirectory: '选择目录…',
    projectLocked: '发送消息后无法更改项目',
    holdClarify: '请先回答上面的问题…',
    holdApproval: '请先处理上面的审批…',
    skippedFiles: '已跳过暂不支持的文件：{{files}}（转成文本或 PDF 后可传）',
  },
  slashMenu: {
    branch: '从当前对话开一个新分支',
    clear: '清空当前对话',
    compact: '压缩历史上下文',
    export: '导出对话为 Markdown',
    memories: '查看 / 管理 Atrium 记住的内容',
    model: '切换当前对话的模型',
    permissions: '调整工具权限',
    planMode: '进入计划模式（只读 / 列计划）',
    search: '搜索过往对话 / 文件 / artifact',
    settings: '打开设置面板',
    stop: '停止当前 agent 运行',
    theme: '切换 light / dark / 跟随系统',
  },
  command: {
    compactDesc: '压缩对话上下文，为模型释放空间',
  },
  commandPalette: {
    placeholder: '搜索对话，或输入命令…',
    groupCommands: '命令',
    groupChats: '对话',
    groupRecent: '最近',
    empty: '没有匹配结果',
    toDark: '切换到深色',
    toLight: '切换到浅色',
    newChat: '新建对话',
    openSettings: '打开设置',
  },
  approval: {
    needConfirm: '需要确认',
    deny: '拒绝',
    allowOnce: '允许一次',
    allowAlways: '总是允许',
    confirmedOnce: '已允许一次',
    confirmedAlways: '已加入信任清单',
    confirmedAlwaysAgent: '已允许，该工具不再询问',
    confirmedDeny: '已拒绝',
    autoReviewed: '自动审查已放行',
    more: '还有 {{count}} 项',
    remember: '总是允许 → 记住',
    reason: {
      network: '联网命令：{{cmd}}',
      dangerous: '危险命令：{{cmd}}',
      substitution: '命令含替换（$() 或反引号），无法静态判定',
      unparseable: '命令无法解析，保险起见需确认',
      wrapper: '包装执行（{{cmd}}），无法静态判定真实命令',
      fsEscape: '写入 workspace 外的路径：{{path}}',
      mcp: 'MCP 服务工具：{{cmd}}',
    },
  },
  clarify: {
    cancelTitle: '取消提问，自己重新输入',
    cancel: '取消',
    hintMulti: '所有问题答完后 Atrium 才会继续',
    hintSingle: '回答后 Atrium 会继续',
    prev: '上一个',
    submit: '提交',
    submitAll: '提交全部',
    next: '下一个',
    otherSingle: '或者填别的…',
    otherMulti: '或者补充别的…',
    text: '自由文本输入…',
    preview: 'Preview · 伪代码',
    answered: '已回答',
    cancelled: '已取消',
    cancelledNote: '已取消提问，未作答。',
  },
  image: {
    generating: '正在生成图片…',
    generated: '生成的图片',
  },
  attachmentViewer: {
    unreadable: '(无法读取内容)',
  },
  home: {
    greetingNight: '夜深了',
    greetingMorning: '早上好',
    greetingNoon: '中午好',
    greetingAfternoon: '下午好',
    greetingEvening: '晚上好',
    greeting: '{{greeting}}，{{name}}',
    greetingNoName: '{{greeting}}',
    getAcquaintedTitle: '认识一下',
    getAcquaintedSub: '我们还没认识 — 聊几句，让我了解你，也想想你希望我是谁。',
    continue: '继续上次',
    newChat: '新对话',
  },
  chat: {
    notFound: '对话不存在',
    serviceNotReady: '聊天服务未就绪',
    more: '更多',
    rename: '重命名',
    archive: '归档',
    archived: '已归档',
    copyAll: '复制全文',
    copiedAll: '已复制全文',
    copyFailed: '复制失败',
    exportMarkdown: '导出为 Markdown',
    exported: '已导出到 {{path}}',
    exportFailed: '导出失败',
    exportUser: '用户',
    exportAssistant: 'AI',
    exportTools: '使用了 {{count}} 个工具',
    exportImage: '图片',
    mermaidSyntaxError: 'Mermaid 语法错误',
    showMore: '展开',
    showLess: '收起',
  },
  tool: {
    verb: {
      read: '读取',
      write: '写入',
      edit: '编辑',
      list: '列出',
      searchGrep: '搜索',
      findFiles: '找到文件',
      run: '运行',
      readOutput: '读取输出',
      stop: '已停止',
      fetch: '抓取',
      searchWeb: '搜索',
      delegate: '委派',
      useSkill: '使用技能',
      genImage: '已生成图片',
      viewImage: '已查看图片',
      memory: '记忆',
      profile: '身份',
      schedule: '已排期',
      cancel: '已取消',
      computerListApps: '已列出应用',
      computerInspect: '已读取',
      computerClick: '已点击',
      computerType: '已输入',
      computerKey: '已按键',
      computerScroll: '已滚动',
      computerDrag: '已拖拽',
      computerSet: '已设置',
      computerAction: '已操作',
    },
    verbActive: {
      read: '读取中',
      write: '写入中',
      edit: '编辑中',
      list: '列出中',
      searchGrep: '搜索中',
      findFiles: '查找文件中',
      run: '运行中',
      readOutput: '读取输出中',
      stop: '停止中',
      fetch: '抓取中',
      searchWeb: '搜索中',
      delegate: '委派中',
      useSkill: '使用技能中',
      genImage: '生成图片中',
      viewImage: '查看图片中',
      memory: '记忆',
      profile: '身份',
      schedule: '排期中',
      cancel: '取消中',
      computerListApps: '正在列出应用',
      computerInspect: '正在读取',
      computerClick: '正在点击',
      computerType: '正在输入',
      computerKey: '正在按键',
      computerScroll: '正在滚动',
      computerDrag: '正在拖拽',
      computerSet: '正在设置',
      computerAction: '正在操作',
    },
    type: {
      file: '文件 · {{path}}',
      directory: '目录 · {{path}}',
      grep: 'Grep · {{path}}',
      glob: 'Glob · {{path}}',
      shell: 'Shell',
      bgShell: '后台 Shell · {{id}}',
      web: 'Web · {{url}}',
      webSearch: '联网搜索',
      subagent: '子智能体 · {{name}}',
      skill: '技能 · {{name}}',
      image: '图片',
      memory: '记忆',
      profile: '身份',
      agent: 'Agent · {{name}}',
      scheduled: '定时任务',
      computer: '电脑操作 · {{app}}',
    },
    memory: {
      view: '查看',
      write: '写入',
      delete: '删除',
    },
    profile: {
      view: '查看',
      write: '写入',
    },
    workspace: '工作区',
    noOutput: '无输出',
    count_one: '{{count}} 个工具',
    count_other: '{{count}} 个工具',
  },
  trace: {
    thought: '已思考',
    worked: '已处理',
    thinking: '思考中',
    working: '处理中',
    duration: '{{verb}} {{duration}}',
    steps_one: '{{count}} 步',
    steps_other: '{{count}} 步',
    tokens: '{{tokens}} tokens',
    plan: '计划',
    compacted: '上下文已压缩',
    compacting: '正在压缩上下文…',
  },
  tokenCounter: {
    tokens: 'Tokens',
    context: '上下文窗口占用',
    input: '输入',
    output: '输出',
    cached: '缓存命中',
    total: '合计',
    inputCost: '输入成本',
    outputCost: '输出成本',
    cacheCost: '缓存成本',
    totalCost: '总成本',
    estimate: '估算值,可能与账单有差异。',
  },
  usage: {
    range: { '7d': '7天', '30d': '30天', month: '本月', year: '今年', all: '全部' },
    cards: {
      totalCost: '总成本',
      totalTokens: '总 Tokens',
      cacheSaved: '缓存省下',
      messages: '消息数',
      cacheHit: '缓存命中',
      calls: '{{count}} 次模型调用',
      inOut: '输入 {{in}} · 输出 {{out}}',
      hitRate: '命中率 {{pct}}%',
      callSplit: '{{chat}} 主对话 · {{sub}} 子代理',
      cachedOf: '{{read}} / {{input}} 命中缓存',
    },
    heatmap: {
      title: 'Token 活跃度',
      desc: '过去一年每日 token 用量',
      total: '过去一年共 {{count}} tokens',
      noActivity: '无活动 ·',
    },
    chart: {
      title: '每日成本',
      desc: '本周期每天花了多少',
      total: '总额',
      byModel: '按模型',
      cost: '成本',
    },
    note: '成本按公开模型单价估算,可能与你的账单有差异。标题/摘要等辅助调用未计入。',
  },
  status: {
    streaming: '生成中',
    done: '完成',
    failed: '失败',
    cancelled: '已取消',
    success: '成功',
    error: '错误',
    warning: '警告',
    running: '运行中…',
  },
  settings: {
    placeholder: '本节内容在后续步骤实装。',
    backToApp: '返回应用',
    sections: {
      generalTitle: '通用',
      identityTitle: '身份',
      appearanceTitle: '外观',
      memoriesTitle: '记忆',
      keyboardTitle: '键盘快捷键',
      usageTitle: '用量与计费',
      providersTitle: '模型',
      skillsTitle: '技能',
      subagentsTitle: '子智能体',
      mcpTitle: 'MCP 服务',
      browserTitle: '浏览器',
      computerTitle: '电脑操作',
      permissionsTitle: '权限',
      hooksTitle: 'Hooks',
      connectionsTitle: '连接',
      worktreesTitle: '工作树',
      archivedTitle: '已归档会话',
      aboutTitle: '关于',
    },
    groups: {
      personal: '个人',
      integrations: '集成',
      coding: '编码',
      archived: '已归档',
    },
    appearance: {
      theme: '主题',
      lightDesc: 'Salon 主题',
      darkDesc: 'Studio 主题',
      systemDesc: '跟随系统外观',
      note: '切换会立即生效。System 模式会跟随 macOS 外观偏好自动变换。',
      textGroup: '文字',
      uiFont: '界面字体',
      uiFontPlaceholder: '例如 Inter',
      uiFontDesc: '应用界面使用的字体。请先在系统中安装该字体；留空则用默认字体。',
      fontSize: '字号',
      fontSizeDesc: '整体缩放所有文字——界面、聊天和代码。',
      fontSizeSmall: '小',
      fontSizeDefault: '默认',
      fontSizeLarge: '大',
      codeGroup: '代码主题',
      codeThemeLight: '浅色主题',
      codeThemeDark: '深色主题',
    },
    browser: {
      lede: '让 AI 在你的 Chrome 里浏览和操作，并且复用你已经登录的网站，在后台工作而不抢占你的浏览器。',
      control: '浏览器控制',
      controlDesc: '允许 AI 导航、点击、输入和读取页面。',
      setup: '设置',
      statusNotConnected: '未连接',
      statusConnected: '已连接',
      statusChromeNotFound: '未检测到 Chrome',
      installChrome: '安装 Google Chrome',
      installChromeDesc:
        '登录模式会复用你日常 Chrome 里的登录态，所以需要装有 Chrome。我们没在这台电脑上找到它。',
      installChromeBtn: '安装 Google Chrome',
      installChromeHint: '使用 Chrome 可以直接复用你的登录信息，用你的身份帮你完成任务',
      installExt: '为 Chrome 添加扩展',
      installExtDesc: 'AI 通过 Playwright 扩展与 Chrome 通信。添加后回到这里，我们会自动检测。',
      installExtBtn: '添加到 Chrome',
      extInstalled: '✓ 已检测到扩展',
      connect: '连接你的浏览器',
      connectDesc: '打开一个你已登录的标签页，然后完成一次性授权——由你选择共享哪个标签页。',
      connectBtn: '连接',
      silentLabel: '重连时跳过授权',
      silentImport: '导入 token →',
      silentEnabled: '✓ 已开启免授权重连',
      tokenWaiting: '等待复制token：在刚打开的扩展页面点「复制」',
      tokenSkip: '跳过，改用浏览器里手动授权',
      tokenError: '这不是有效的 token，请在扩展页面点「复制」。',
      connectionTitle: '连接',
      connectionSession: 'Chrome · 你的登录会话',
      connectionSessionDesc: '正在复用你已登录的标签页。会自动重连，无需再次授权。',
      reconnect: '重新连接',
      disconnect: '断开',
      extensionLinked: '扩展已连接',
      tabGroupNote: '共享标签组：Playwright · 把标签页拖进去即可交给 AI 使用',
      modesTitle: 'AI 如何选择浏览器',
      modePublic: '公开浏览',
      modePublicDesc: 'AI 打开新的 Chrome 实例，不碰你原本的Chrome，但是无法使用你的登录信息。',
      modeSignedIn: '登录浏览',
      modeSignedInDesc: '配合 Chrome 插件，AI 可以使用你的登录信息来完成任务。',
      modeReady: '就绪',
      modeNeedsSetup: '待设置',
      modeNeedsChrome: '需要 Chrome',
      bannerNote:
        'AI 工作时，Chrome 顶部会出现一条「… 正在调试此浏览器」的横幅。这是正常的请放心。',
    },
    computer: {
      lede: '让 AI 像人一样看屏幕、点击和输入，直接操作 Mac 上的原生应用。完成网页做不到的桌面任务。网页任务仍由「浏览器控制」处理。',
      enable: '启用电脑操作',
      enableDesc: '开启后，AI 在获得授权的应用内可执行点击、输入等操作。你可随时关闭。',
      comingSoon: 'Windows & Linux 即将支持',
      comingSoonDesc: '电脑操作目前仅支持 macOS，其他平台的支持正在路上。',
      permissionsTitle: '系统授权',
      permReady: '已就绪',
      permNeeded: '待授权',
      accessibility: '辅助功能',
      accessibilityDesc: '允许 AI 读取界面结构并模拟点击、键盘操作。',
      screenRecording: '屏幕录制',
      screenRecordingDesc: '允许 Atrium 截取目标应用窗口，让 AI 能看到界面。',
      granted: '已授权',
      grant: '授权',
      restarting: '已授权，正在重启 Atrium…',
      restartNote: '授权完成后，Atrium 会自动重启以生效。',
      dragTitle: '授权「{{name}}」',
      dragDesc: '已打开系统设置。把下面这块拖进列表即可授权 —— 不用手动点「+」或找文件。',
      dragName: 'Atrium',
      dragHint: '按住拖动 →',
      dropHead: '{{name}} · 系统设置',
      dropHint: '拖到系统设置的列表里松手，自动添加并开启',
      dragClose: '稍后',
      behaviorTitle: '行为与安全',
      menubar: '在菜单栏显示操作状态',
      menubarDesc: 'AI 操作时，菜单栏出现「目标应用 + 鼠标」图标显示它在做什么，点击可停止。',
      pause: '我操作键鼠时立即暂停',
      pauseDesc: '检测到你在动鼠标或键盘，立刻暂停 AI，避免互相干扰。',
      confirm: '敏感操作需二次确认',
      confirmDesc: '提交、支付、删除等不可逆动作执行前，先弹出确认。',
    },
    about: {
      description:
        'Atrium 是一款本地优先的桌面 AI Agent 助手。支持携带密钥接入任意模型提供商，涵盖目前市面常见的 AI Agent 功能。',
      craftedBy: '由 <author>lhz960904</author> 打造',
      lastChecked: '上次检查',
      never: '从未',
      checkNow: '检查更新',
      checking: '检查中…',
      upToDate: '已是最新版本。',
      available: '发现新版本 {{version}}',
      downloadingStatus: '下载中 {{percent}}%',
      readyStatus: '{{version}} 已下载，可安装',
      checkFailed: '检查更新失败',
      feedback: '发送反馈',
      feedbackDesc: '在 GitHub 上反馈问题或提出新功能。',
    },
    language: {
      title: '语言',
      zh: '中文',
      en: 'English',
      system: '跟随系统',
      systemDesc: '跟随系统语言',
      note: '切换后界面文案会立即更新。',
    },
    general: {
      groupGeneral: '通用',
      language: '语言',
      languageDesc: '界面语言。跟随系统时使用操作系统语言。',
      defaultModel: '默认会话模型',
      defaultModelDesc: '新会话默认使用的模型；可在会话内单独更改。',
      defaultModelAuto: '自动（第一个可用）',
      autoTitle: '自动生成会话标题',
      autoTitleDesc: '新会话时把首条消息总结成标题。',
      launchAtLogin: '开机自动启动',
      launchAtLoginDesc: '登录后自动打开 Atrium。',
      showInMenuBar: '显示在菜单栏',
      showInMenuBarDesc: '在 macOS 菜单栏常驻 Atrium 图标，便于快速操作。',
      groupComposer: '输入框',
      sendKey: '发送消息',
      sendKeyDesc: '选择哪个按键发送，其余按键用于换行。',
      sendKeyEnter: 'Enter 发送 · Shift+Enter 换行',
      sendKeyMod: '{{mod}}+Enter 发送 · Enter 换行',
      sendKeyShift: 'Shift+Enter 发送 · Enter 换行',
      hideTokenUsage: '隐藏 Token 占用',
      hideTokenUsageDesc: '隐藏输入框工具栏中的 Token 与上下文窗口占用展示。',
    },
    identity: {
      you: '你',
      youHint: '这里的 name 决定首页怎么称呼你。',
      assistant: '你的助手',
      assistantHint: '注入 system prompt 的人格。',
      notSet: '还没设置。',
      reacquaint: '重新认识一下',
      reacquaintHint: '再跑一次对话式 onboarding，覆盖这两份。',
      emptyTitle: '还没互相认识',
      emptyBody: '聊几句，让我了解你、也定下我该是谁。之后随时回这里编辑。',
      start: '认识一下',
    },
    memories: {
      scopeGlobal: '全局',
      count: '{{n}} 条',
      empty: '还没有记忆。聊天时我会在值得记住的事情上自己写进来——你不用手动加。',
      deleteConfirm: '删除这条记忆？',
    },
    archived: {
      count: '{{n}} 个已归档',
      empty: '还没有已归档的会话。',
      searchPlaceholder: '搜索已归档会话',
      noResults: '没有匹配的会话。',
      restore: '恢复',
      restored: '已恢复会话',
      deleteConfirm: '永久删除这个会话？此操作无法撤销。',
    },
    keyboard: {
      colCommand: '命令',
      colKeybinding: '快捷键',
      cmdSearch: '搜索',
      cmdNewChat: '新建对话',
      cmdToggleSidebar: '切换侧边栏',
      cmdOpenSettings: '打开设置',
      cmdCloseWindow: '关闭窗口',
      cmdQuit: '退出',
      descSearch: '搜索对话与命令',
      descNewChat: '开始一个新对话',
      descToggleSidebar: '显示或隐藏侧边栏',
      descOpenSettings: '打开设置窗口',
      descCloseWindow: '关闭当前窗口',
      descQuit: '退出 Atrium',
      pressShortcut: '按下快捷键',
      cancel: '取消',
      reset: '恢复默认',
      needModifier: '快捷键需包含 ⌘ 或 Ctrl',
      usedBy: '已被「{{cmd}}」占用',
      systemManaged: '由系统菜单管理',
    },
    permissions: {
      mode: '权限模式',
      modeDefaultLabel: '默认权限',
      modeDefaultDesc: '越界操作（联网 / 外部写 / 危险命令）需你确认',
      modeReviewLabel: '自动审查',
      modeReviewDesc: '越界操作先交 reviewer 模型判定',
      modeFullLabel: '完全放行',
      modeFullDesc: '不拦截任何操作',
      reviewer: 'Reviewer 模型',
      reviewerHint:
        '自动审查模式用它判断越界操作是否安全。留空则用当前对话模型（外部 agent 对话无可用模型时退回弹窗）。',
      reviewerNone: '未配置（用对话模型）',
      trustList: '信任清单',
      trustHint:
        '这些操作越界时不再询问。多数会在审批卡点「总是允许」时自动加入；也可在此手动添加。',
      addPlaceholder: '命令或路径，如 curl、npm install、~/.ssh/config',
      add: '添加',
      willAllow: '将允许：',
      allowCommand: '允许所有 {{cmd}} 命令',
      allowPath: '允许写入 {{path}}',
      empty: '还没有信任的操作。',
    },
    providers: {
      active: '已启用',
      notConfigured: '未配置',
      models: '模型',
      fetch: '拉取',
      enabledCount: '已启用 {{on}} / {{total}}',
      apiKey: 'API Key',
      getKeyFrom: '在此获取 API key:',
      baseUrl: 'Base URL',
      optional: '(可选)',
      command: '命令',
      arguments: '参数',
      baseUrlHint: '留空使用默认 endpoint。',
      fetchHint: '点 Fetch 拉取可用模型。',
      fetchHintNoKey: '填好 API key 后即可 Fetch 模型。',
      localNoKey: '无需 API key —— 复用你本地已登录的 CLI。',
      localInstallHint: '先全局安装它的 CLI / ACP 适配器:',
      localAfterInstall: '安装并登录后(如 {{login}}),启用本项,即可在对话框的模型选择里选它。',
      localCommandHint: '启动 ACP agent 的命令,留空用默认。',
      localArgsHint: '空格分隔的命令行参数,留空用默认。',
      desc: {
        anthropic: 'Claude 系列模型 — Opus、Sonnet、Haiku。官方 API 直连。',
        openai: 'GPT-5、GPT-4.1 与 o 系列,经 OpenAI API。',
        deepseek: 'DeepSeek-V3 / R1。性价比高,擅长代码与推理。',
        google: 'Gemini 2.5 Pro / Flash,经 Google AI Studio。',
        moonshot: 'Kimi 系列（Moonshot AI）。长上下文。',
        kimiCoding: '月之暗面 · Coding 订阅版（Anthropic 兼容）。',
        zaiCoding: '智谱清言 GLM Coding 订阅版（Anthropic 兼容）。',
        openrouter: '统一 API 路由 300+ 模型。',
        aihubmix: '一站式 LLM 聚合，单 key 调多家模型。',
        claudeCode: '通过 ACP 调用你本地已登录的 Claude Code（复用订阅，无需 API key）。',
        codexCli: '通过 ACP 调用你本地已登录的 Codex CLI（复用 ChatGPT 订阅）。',
        geminiCli: '通过 ACP 调用你本地已登录的 Gemini CLI（自带 ACP）。',
        ollama: '本地运行开源模型（Qwen、Gemma、DeepSeek 等）。检测到服务即可用，无需 API key。',
      },
      localService: {
        running: '服务运行中 · v{{version}}',
        notRunning: '未检测到本地服务',
        installHint: '安装并启动 Ollama 后会自动检测到：',
        emptyModels: '没有检测到已安装的模型。',
        downloadTitle: '下载模型',
        downloadHint: '完整目录：',
        download: '下载',
        customPlaceholder: '输入模型名，如 qwen3:4b',
        verifying: '验证中…',
        foundSize: '✓ 可下载 · {{size}}',
        foundCloud: '✓ 云端托管模型 · 无本地权重，推理跑在 ollama.com（需登录 Ollama 账号）',
        notFound: '✗ 模型库中没有这个名字',
        verifyFailed: '无法连接模型库验证，仍可尝试下载',
      },
    },
    subagents: {
      systemPrompt: '系统提示词',
      newSubagent: '新建 Subagent',
      builtin: '内置',
      availableTools: '可用工具',
      inheritTools: '继承主 agent 的全部工具',
      handoffModel: '承接模型',
      inheritModel: '继承主对话模型',
      emptyHint: '选择左侧的 subagent，或新建一个。',
      name: '名称',
      namePlaceholder: '例如 code-reviewer',
      descLabel: '描述（主 agent 据此决定何时委派）',
      limitTools: '限制可用工具（默认继承主 agent 的全部工具）',
    },
    mcp: {
      name: '名称',
      namePlaceholder: 'MCP 服务名称',
      stdio: 'STDIO',
      http: 'Streamable HTTP',
      command: '启动命令',
      args: '参数',
      addArg: '添加参数',
      env: '环境变量',
      envNote: '加密存储；设置到服务进程的环境里。',
      addEnv: '添加环境变量',
      envPassthrough: '透传环境变量',
      envPassthroughNote: '从 Atrium 自身环境转发的变量名。',
      addVar: '添加变量',
      cwd: '工作目录',
      url: 'URL',
      bearerTokenEnvVar: 'Bearer Token 环境变量',
      headers: '请求头',
      headersNote: '加密存储；每次请求都会带上。',
      addHeader: '添加请求头',
      headersFromEnv: '来自环境变量的请求头',
      headersFromEnvNote: '值为连接时读取的宿主环境变量名。',
      key: 'Key',
      value: 'Value',
      enabled: '启用',
      addServer: '添加 MCP 服务',
      editServer: '编辑 {{name}}',
      count: '已配置 {{count}} 个',
      fromPlugins: '来自插件',
      emptyList: '还没有 MCP 服务。',
      emptyHint: '选择左侧的 MCP 服务，或添加一个。',
      authenticate: '授权',
      authenticating: '授权中…',
      attentionBadge: '部分服务需要处理',
      attentionToast: '{{count}} 个 MCP 服务需要处理',
      attentionToastAction: '查看',
      status: {
        connected: '已连接',
        'needs-auth': '需要授权',
        error: '连接失败',
        connecting: '连接中…',
        disabled: '已禁用',
      },
      json: {
        button: '编辑 JSON',
        title: '以 JSON 编辑 MCP 服务',
        description: '从其他 AI 客户端导入，或把所有服务作为一份 JSON 文档编辑。',
        diffToggle: '对比',
        importFrom: '导入自',
        fromFile: '从文件…',
        fixJsonFirst: '请先修正 JSON 语法再导入。',
        reset: '重置为当前',
        apply: '应用',
        applying: '应用中…',
        notes: '提示',
      },
    },
    skills: {
      empty: '没有发现 skill。把 {{file}} 目录放进 {{dirA}}、{{dirB}} 或 {{dirC}} 后重启。',
      filter: '筛选 {{count}} 个 skill…',
      noMatch: '没有匹配的 skill。',
      noContent: '无内容。',
    },
  },
};
