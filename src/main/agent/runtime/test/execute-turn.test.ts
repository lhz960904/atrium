...
async function runOnce(text: string, overrides: Partial<Parameters<typeof executeTurn>[0]> = {}) {
...
test('settlement cleanup runs once even when final bookkeeping fails', async () => {
  let settled = 0;
  await expect(
    runOnce('done', {
      recordUsage: () => {
...
test('settlement cleanup also runs when preparing tools fails', async () => {
  let settled = 0;
  await expect(
    runOnce('done', {
      buildTools: () => {
...
test('cleanup failure does not replace the original execution error', async () => {
