...
const MEMORY_ENTRY_FIELDS = {
  ...
  description: Type.Optional(
    Type.String({ description: 'one-line summary shown in the index (required on write)' }),
  ),
  type: Type.Optional(
  ...
  body: Type.Optional(
    Type.String({
      description: 'the memory itself — what to remember and why it matters (required on write)',
    }),
  ),
};

const COMMAND = StringEnum(['view', 'write', 'delete']);
...
export function memoryTool(ctx: ToolCtx) {
  return defineTool({
    ...
    parameters: Type.Object({
      command: COMMAND,
      // The schema default is advisory only — the engine validates arguments
      // but never fills defaults, so scope is resolved again below.
      scope: Type.Optional(StringEnum(['project', 'global'], { default: 'project' })),
      ...MEMORY_ENTRY_FIELDS,
    }),
    execute: async (_id, { command, scope, name, description, type, body }) =>
      textResult(
        await dispatchMemory(memoryDir(scope ?? 'project', ctx.workspaceRoot), {
          command,
          name,
          description,
          type,
...
export function memoryDirTool(dir: string) {
  return defineTool({
    ...
    parameters: Type.Object({ command: COMMAND, ...MEMORY_ENTRY_FIELDS }),
    execute: async (_id, input) => textResult(await dispatchMemory(dir, input)),
  });
}
