 ...
 */
const sizable = (messages: Message[]) => convertToLlm(messages);

/** Size one native Agent message, including pi's summary and custom message roles. */
export function tokensOfMessage(message: Message): number {
  return convertToLlm([message]).reduce(
    (total, converted) => total + tokensOfModelMessage(converted),
    0,
  );
}

function tokensOfModelMessage(message: import('@earendil-works/pi-ai').Message): number {
...
export function countTokens(messages: Message[]): number {
  ...
  let tail = 0;
  for (let i = anchor + 1; i < sized.length; i++) tail += tokensOfModelMessage(sized[i]);
  return base + tail;
}
...
export function estimateContextTokens(messages: Message[]): number {
  let total = 0;
  for (const message of sizable(messages)) total += tokensOfModelMessage(message);
  return total;
}
