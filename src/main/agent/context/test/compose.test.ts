...
import { type ContextTransform, composeContext } from '../compose';
...
test('context transform lists contain only functions', () => {
  expectTypeOf<Parameters<typeof composeContext>[0]>().toEqualTypeOf<ContextTransform[]>();
});

test('an empty transform list leaves messages unchanged', async () => {
  const messages = [user('start')];
  expect(await composeContext([])(messages)).toBe(messages);
});
