...
export function renderTranscript(messages: Message[]): string {
  return convertToLlm(messages).map(renderMessage).join('\n\n');
}
...
export const foldedView = (fold: Fold): Message[] => [
  createCompactionSummaryMessage(fold.summary, fold.tokensBefore, Date.now()),
  ...fold.retainedTail,
];
...
export function withinTurnFold(opts: {
  ...
  return async (input) => {
    ...
    if (tokens < window * COMPACT_AT_RATIO) return base;
    ...
    try {
      ...
      return [summary, ...selected.recent];
    } catch (err) {
...
export function contextCompaction(options: Parameters<typeof withinTurnFold>[0]): HookSet {
